...
public class ProductService {
    ...
    public DeferredResult<ResponseEntity<List<ProductDto>>> list() {
        final DeferredResult<ResponseEntity<List<ProductDto>>> dr = new DeferredResult<>();

        List<ProductDto> produtos = repository.findAll().stream().map(ProductDto::new).toList();

        dr.setResult(ResponseEntity.status(200).body(produtos));
